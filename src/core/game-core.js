export function initChinaleApp(gameList, options = {}) {
  const MAX_ATTEMPTS = 6;
  const games = gameList.filter((game) => game?.id);
  const DEFAULT_GAME_ID = options.defaultGameId || games[0]?.id || "";
  const GAME_CONFIGS = buildGameConfigMap(games);
  ...
  const subdivisionBoundaryCache = new Map();
  const taiwanTownBoundaryCache = new Map();
  let taiwanBoundaryPromise = null;

  const els = {};
  let activeGame = GAME_CONFIGS[DEFAULT_GAME_ID];
  let map = null;
  ...
  let atlasSubdivisionLayerGroup = null;
  let atlasSeatLayerGroup = null;
  ...
  function buildGameConfigMap(sourceGames) {
    ...
    sourceGames.forEach((game) => {
      configMap[game.id] = game;
      (game.aliases || []).forEach((alias) => {
        configMap[alias] = game;
      });
    });
    return configMap;
  }
        ...
        (game, index) => `
          <button class="game-card" type="button" data-game-id="${escapeHtml(game.id)}" style="--choice-delay: ${80 + index * 54}ms">
            ...
            <span class="game-card-main">
              ...
              <span>${escapeHtml(game.cardSummary || game.initialText || "")}</span>
            </span>
  ...
  function selectGame(gameId, options = {}) {
    ...
    if (switchedGame || !state.target || state.finished) {
      startGame(getDailyTarget(activeGame), "daily", shanghaiDateKey());
    } else {
  ...
  function updateGameChrome() {
    els.gameTitle.textContent = activeGame.title;
    els.gameEyebrow.textContent = activeGame.eyebrow;
    const badgeIcon = els.targetMode?.previousElementSibling;
    if (badgeIcon) {
      badgeIcon.remove();
      els.targetMode.insertAdjacentHTML("beforebegin", `<i data-lucide="${activeGame.badgeIcon}"></i>`);
    }
  ...
  function setInitialFeedback() {
    setFeedback(activeGame.initialIcon, activeGame.initialTitle, activeGame.initialText);
  }

  function initMap() {
    if (!window.L) {
      els.map.innerHTML = `<div class="history-empty">${escapeHtml(activeGame.loadFailText)}</div>`;
      setFeedback("triangle-alert", activeGame.loadFailTitle, activeGame.loadFailText);
      return;
    }
    ...
    atlasSubdivisionLayerGroup = L.layerGroup().addTo(map);
    atlasSeatLayerGroup = L.layerGroup().addTo(map);
  ...
  function createMapPanes() {
    ["atlasPane", "boundaryPane", "atlasPointPane", "boundaryLabelPane"].forEach((paneName) => {
      if (!map.getPane(paneName)) {
        map.createPane(paneName);
      }
    });

    const atlasPane = map.getPane("atlasPane");
    const boundaryPane = map.getPane("boundaryPane");
    const atlasPointPane = map.getPane("atlasPointPane");
    const labelPane = map.getPane("boundaryLabelPane");
    atlasPane.style.zIndex = 420;
    atlasPane.style.pointerEvents = "none";
    boundaryPane.style.zIndex = 430;
    boundaryPane.style.pointerEvents = "none";
    atlasPointPane.style.zIndex = 438;
    atlasPointPane.style.pointerEvents = "none";
  ...
  function configureMapForActiveGame() {
    ...
    const usesSatellite = activeGame.mapMode === "satellite";
  ...
  function submitGuess(guess, source = "input") {
    ...
    if (activeGame.mapMode === "satellite") {
  ...
  async function renderAtlasTarget(token) {
    if (!map || !window.L || !state.target || !atlasSubdivisionLayerGroup || !atlasSeatLayerGroup) {
      return;
    }

    const target = state.target;
    const [targetGeojson, subdivisionGeojson] = await Promise.all([
      loadBoundary(target),
      loadSubdivisionBoundary(target),
    ]);
    if (token !== boundaryRenderToken || target !== state.target) {
      return;
    }

    const sourceGeojson = subdivisionGeojson || targetGeojson;
    if (!sourceGeojson) {
      setFeedback("triangle-alert", "版图暂时不可用", `${target.name} 的行政版图没有加载成功。`);
      return;
    }
    ...
    const subdivisionLayer = L.geoJSON(sourceGeojson, {
      pane: "atlasPane",
      interactive: false,
      className: "atlas-county-path",
      style: getAtlasFeatureStyle,
    }).addTo(atlasSubdivisionLayerGroup);
    ...
    const features = getGeoJsonFeatures(sourceGeojson);
    renderAtlasSeatMarkers(features.length ? features : getGeoJsonFeatures(targetGeojson), target);

    const layerBounds = targetBoundaryLayer.getBounds?.() || subdivisionLayer.getBounds?.();
    atlasTargetBounds = layerBounds?.isValid?.() ? layerBounds.pad(0.08) : null;
    targetBoundaryLayer.bringToFront();
  ...
  function getAtlasFeatureStyle(feature) {
    const palette = getAtlasPalette(getCountyCategory(feature));
    return {
  ...
  function getAtlasPalette(category) {
    if (category === "district") {
      return { stroke: "#5fd4ff", fill: "#5fd4ff" };
    }
    if (category === "countyCity") {
      return { stroke: "#b987ff", fill: "#a46bff" };
    }
    return { stroke: "#ffd166", fill: "#ffcf66" };
  }

  function renderAtlasSeatMarkers(features, fallbackCity) {
    const markerFeatures = features.length ? features : [{ properties: {}, geometry: null }];
    markerFeatures.forEach((feature) => {
      const point = getFeatureSeatLatLng(feature, fallbackCity);
      if (!point) {
        return;
      }

      const category = getCountyCategory(feature);
      L.marker(point, {
        pane: "atlasPointPane",
        interactive: false,
        keyboard: false,
        icon: L.divIcon({
          className: "",
          html: `<span class="county-seat-dot county-seat-${category}"></span>`,
          iconSize: [14, 14],
          iconAnchor: [7, 7],
        }),
      }).addTo(atlasSeatLayerGroup);
    });
  }

  function getFeatureSeatLatLng(feature, fallbackCity) {
    const properties = feature?.properties || {};
    const coordinate =
      parseLngLat(properties.center) ||
      parseLngLat(properties.centroid) ||
      parseLngLat(properties.cp);
    if (coordinate) {
      return [coordinate.lat, coordinate.lng];
    }

    if (feature?.geometry) {
      return getGeoJsonCentroid(feature, fallbackCity);
    }

    return fallbackCity ? [fallbackCity.lat, fallbackCity.lng] : null;
  }
  ...
  function getCountyCategory(feature) {
  ...
  function getGeoJsonFeatures(geojson) {
    ...
    if (geojson.type === "FeatureCollection") {
      return Array.isArray(geojson.features) ? geojson.features.filter((feature) => feature?.geometry) : [];
    }
  ...
  async function loadSubdivisionBoundary(city) {
    const code = String(city?.code || "");
    if (!code) {
      return null;
    }

    if (getTaiwanTownBoundaryUrl(code)) {
      return loadTaiwanSubdivisionBoundary(code).then((geojson) => geojson || loadBoundary(city));
    }

    if (!subdivisionBoundaryCache.has(code)) {
      subdivisionBoundaryCache.set(
        code,
        fetch(getBoundaryUrl(code, { full: true }))
          .then((response) => {
            if (!response.ok) {
              throw new Error(`Boundary ${code}_full returned ${response.status}`);
            }
            return response.json();
          })
          .then((geojson) => (hasGeoJsonGeometry(geojson) ? geojson : null))
          .catch((error) => {
            console.warn("县级行政区版图加载失败", city?.name || code, error);
            return loadBoundary(city);
          }),
      );
    }

    return subdivisionBoundaryCache.get(code);
  }

  async function loadTaiwanSubdivisionBoundary(code) {
    const url = getTaiwanTownBoundaryUrl(code);
    if (!url) {
      return null;
    }

    const cacheKey = `${activeGame.id}:${code}`;
    if (!taiwanTownBoundaryCache.has(cacheKey)) {
      taiwanTownBoundaryCache.set(
        cacheKey,
        fetch(url)
          .then((response) => {
            if (!response.ok) {
              throw new Error(`Taiwan town boundary ${code} returned ${response.status}`);
            }
            return response.json();
          })
          ...
          .then((geojson) => (hasGeoJsonGeometry(geojson) ? geojson : null))
          .catch((error) => {
            console.warn("台湾区级版图加载失败", code, error);
            return null;
          }),
      );
    }

    return taiwanTownBoundaryCache.get(cacheKey);
  }

  function getTaiwanTownBoundaryUrl(code) {
    return activeGame?.taiwanTownBoundaryByCode?.[code] || null;
  }

  function getBoundaryUrl(code, options = {}) {
    const suffix = options.full ? "_full" : "";
    if (shouldUseBoundaryProxy()) {
      return `/api/boundary?code=${encodeURIComponent(code)}${options.full ? "&full=1" : ""}`;
    }

    return `${BOUNDARY_DIRECT_BASE_URL}/${code}${suffix}.json`;
  }
  ...
  function setDirectionalFeedback(icon, title, distance, bearing, stepLabel) {
    ...
    els.feedbackTitle.textContent = title;
    const progressText =
      typeof activeGame.progressText === "function"
        ? activeGame.progressText(escapeHtml(stepLabel))
  ...
  function getCurrentAllowedBounds() {
    ...
    if (activeGame.mapMode === "atlas" && atlasTargetBounds?.isValid?.()) {
      return atlasTargetBounds;
    }
  ...
  function getDailyTarget(game = activeGame) {
    if (!cities.length) {
      return null;
    }

    const key = shanghaiDateKey();
    const hashKey = game.id === DEFAULT_GAME_ID ? key : `${game.id}:${key}`;
    return cities[hashString(hashKey) % cities.length];
  }
  ...
  function loadStats(game = activeGame) {
    try {
      const storageText =
        localStorage.getItem(game.storageKey) ||
        (game.legacyStorageKeys || [])
  ...
  function saveStats() {
    try {
      localStorage.setItem(activeGame.storageKey, JSON.stringify(stats));
    } catch {
  ...
  function buildShareText() {
    ...
    return [
      `${activeGame.shareName} ${cities.length} 城 ${state.mode === "daily" ? state.puzzleKey : "随机"}`,
      `成绩 ${result}`,
      ...rows,
